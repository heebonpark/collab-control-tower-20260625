...
function mapCategoryRow(r, itemRows = []) {
  return {
    id: r.id, name: r.name, color: r.color,
    items: itemRows.map((i) => ({ id: i.id, name: i.name, cycle: i.cycle })),
  };
}
...
export async function fetchCategories() {
  ...
  return data.map((row) =>
    mapCategoryRow(row, [...row.category_items].sort((a, b) => a.sort_order - b.sort_order))
  );
}
...
export async function createCategoryItem({ categoryId, name, cycle }) {
  const { data, error } = await supabase.from("category_items").insert({
    category_id: categoryId, name, cycle,
  }).select().single();
  if (error) throw error;
  return { id: data.id, name: data.name, cycle: data.cycle };
}
...
export async function updateTaskDetails(taskId, { priority, due, desc, unitId, role, owner }, actor) {
  const { data, error } = await supabase.from("tasks").update({
    priority, due, description: desc, unit_id: unitId, role, owner,
  }).eq("id", taskId).select().single();
  if (error) throw error;
  const log = await insertLog(taskId, "업무 내용이 수정되었습니다", actor);
  await notifyUnit(`"${data.title}" 업무 내용이 수정됨`, taskId, unitId, "update");
...
export async function uploadAttachment(taskId, file, actor) {
  ...
  const { error: uploadError } = await withTimeout(
    supabase.storage.from(ATTACHMENTS_BUCKET).upload(filePath, file),
    20000,
    "업로드 시간이 초과되었습니다. 회사 네트워크 방화벽이 차단하고 있을 수 있습니다."
  );
